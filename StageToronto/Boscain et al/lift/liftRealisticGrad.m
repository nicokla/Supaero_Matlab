function [image3d, renormalizationMatrix] = liftRealisticGrad...
    (image, thetaNumber, anglesExacts, lambda1, ...
    lambda2, canBeLifted, typeProjection)
global good;
[Mx,My]=size(image);
    ...
    for j=1:My
        if(good(i,j))
            if canBeLifted(i,j)
                ...
                bigSide=sqrt(lambda1(i,j));
                littleSide=sqrt(lambda2(i,j));
                
                v=(littleSide*bigSide*ones(size(theta))./...
                sqrt((littleSide^2*cos(theta-theta0).^2+...
                bigSide^2*sin(theta-theta0).^2)));

%                 projection=sum(v);
%                 v=v*image(i,j)/projection;
