function imageRGB = criminisiInpainting_specialRGB(image, good, semiSize, display)
% todo : 1) debug (noir)
...
for i=deltaX
    for j=deltaY
        if(~good(cx+i,cy+j))
            ...
            imageRGB(cx+i,cy+j,:)=imageRGB(x0+i,y0+j,:);
        end
    end
end
...
if(display)
    figure(2),sc(imageRGB); title(num2str(etape));
    drawnow;
end
