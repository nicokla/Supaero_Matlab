...
/*double getDist_old(Mat3d &image3d1, Mat3d &image3d2, int i, int j, int a, int b){
	...
	for(n=0; n < image3d1.Mz; n++){
        x=image3d1.getNb(i,j,n);
        y=image3d2.getNb(a,b,n);
        
        if(x>0 && y>0){
            temp=x*log(x/y) + y*log(y/x);
            d += temp;
        }
...
double getDist(Mat3d &image3d1, Mat3d &image3d2, int i,
        int j, int a, int b, double distMax){ //int step11, int nbStep11
	double temp, d=0;
	double x, y;
	int n, n2, n3=0;
	for(n=0; n<nbStep11; n++){
		for(n2=0; n2<step11; n2++){
            x=image3d1.getNb(i,j,n3);
            y=image3d2.getNb(a,b,n3);
            if(x>0 && y>0){
                d += x*log(2*x/(y+x));
                d += y*log(2*y/(y+x));
            }
...
step11=max(1, image3d1.Mz/5);
nbStep11=image3d1.Mz/step11;
		...
		for(l=0; l<k; l++){
			while(1){
				...
				if (chosen.try_insert(a, b) && 
                    ...
                    d = getDist(image3d1, image3d2, i, j, a, b, DBL_MAX);
