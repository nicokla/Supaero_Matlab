...
function image_debruitee=debruitage_2(T_0,q_max,alpha,beta,x,nb_classes)
...
% A DECOMMENTER
pas=256/nb_classes;
moyennes = (pas/2:pas:(pas/2+pas*(nb_classes-1)))';
variances = pas/2 * ones(nb_classes,1);
moyennes_variances= [ moyennes variances ];
...
for q = 1:q_max
	for i = 1:nb_lignes
		for j = 1:nb_colonnes
			...
			energie_courante = (x(i,j)-moyennes_variances(classe_courante,1)).^2;
            energie_nouvelle = (x(i,j)-moyennes_variances(classe_nouvelle,1)).^2;

            % Energie correspondant a l'a priori (modele de Potts 4-connexe) :
			if i>1
				energie_courante = energie_courante+beta*(moyennes_variances(classe_courante,1) - image_debruitee(i-1,j)).^2;
				energie_nouvelle = energie_nouvelle+beta*(moyennes_variances(classe_nouvelle,1) - image_debruitee(i-1,j)).^2;
			end
			if j>1
				energie_courante = energie_courante+beta*(moyennes_variances(classe_courante,1) - image_debruitee(i,j-1)).^2;
				energie_nouvelle = energie_nouvelle+beta*(moyennes_variances(classe_nouvelle,1) - image_debruitee(i,j-1)).^2;
			end
			if i<nb_lignes
				energie_courante = energie_courante+beta*(moyennes_variances(classe_courante,1) - image_debruitee(i+1,j)).^2;
				energie_nouvelle = energie_nouvelle+beta*(moyennes_variances(classe_nouvelle,1) - image_debruitee(i+1,j)).^2;
