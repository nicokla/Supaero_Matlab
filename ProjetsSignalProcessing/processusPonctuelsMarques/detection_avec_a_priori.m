...
% Parametres divers :
N = 186;									% Nombre de disques d'une configuration
...
for q = 1:q_max
    ...
    for aa=1:N
        if((((abscisses_centres_disques(aa) - abscisse_centre_disque)^2+...
		(ordonnees_centres_disques(aa) - ordonnee_centre_disque)^2) <= facteur*R_au_carre) && aa ~= k)
            energie_nouvelle=0;
            break;
        end
    end
